...
def MemoryGet(temp,six=False):
    ...
    if not(str(tmp) in Memory):
        return "00000000"
    if six:
        return Memory[str(tmp)]+Memory[str(tmp+1)]+Memory[str(tmp+2)]+Memory[str(tmp+3)]+Memory[str(tmp+4)]+Memory[str(tmp+5)]
    s=Memory[str(tmp)]
    if (s[0]=="3") or (s[0]=="4") or (s[0]=="5") or (s[0]=="c"):
        return Memory[str(tmp)]+Memory[str(tmp+1)]+Memory[str(tmp+2)]+Memory[str(tmp+3)]+Memory[str(tmp+4)]+Memory[str(tmp+5)]
    elif (s[0]=="7") or (s[0]=="8"):
        return Memory[str(tmp)]+Memory[str(tmp+1)]+Memory[str(tmp+2)]+Memory[str(tmp+3)]+Memory[str(tmp+4)]
    elif (s[0]=="2") or (s[0]=="6") or (s[0]=="a") or (s[0]=="b"):
        return Memory[str(tmp)]+Memory[str(tmp+1)]
    elif (s[0]=="0") or (s[0]=="1") or (s[0]=="9") or (s[0]=="d"):
        return Memory[str(tmp)]
    else:
        return ""
#alu-------------------------------------------------------
def GetBin(k):
